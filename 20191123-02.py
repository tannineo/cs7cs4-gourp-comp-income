...
t_df.drop(t_df[t_df['Income'] < 0].index, inplace=True)
...
total_df.drop(['Glasses', 'Hair', 'Height'], axis=1, inplace=True)
...
def targetEncoding(training_set, target_variable, cat_cols, min_sample_leaf,
                   alpha, noise_level):
    ...
    for column in cat_cols:
        ...
        reg_smooth_val = ((target_cat_mean * cat_count) +
                          (globalmean * alpha)) / (cat_count + alpha)
...
y_with_addin = (
    y_ttl[y_ttl['AddIn'] > 0]['Income']).map(lambda y: np.log(y + 1))
y_without_addin = y_ttl[y_ttl['AddIn_0'] == 1]['Income'].map(
    lambda y: np.log(y + 1))
...
# TODO: lgb build-in cross validation
...
X_trainval, X_test, y_trainval, y_test = train_test_split(X_with_addin,
                                                          y_with_addin,
                                                          test_size=0.2,
                                                          random_state=None)
...
X_trainval, X_test, y_trainval, y_test = train_test_split(X_without_addin,
                                                          y_without_addin,
                                                          test_size=0.2,
                                                          random_state=None)
trn_data = lgb.Dataset(X_trainval, label=y_trainval)
